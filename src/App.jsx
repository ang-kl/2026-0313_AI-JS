// v1.5.4 - 2026-03-18 - Chat 6 - deduplicate picker results at all call sites, nudge button padding fix
  ...
  const doSearch = useCallback(async () => {
    ...
    try {
      ...
      searchOccupations(tidyQuery, "15 to 20").then(full => {
        ...
        // Deduplicate: merge quick and full, keeping first occurrence of each title
        const seen = new Set(quick.map(o => o.title.toLowerCase()));
        const additional = full.filter(o => {
          const k = o.title.toLowerCase();
          if (seen.has(k)) return false;
          seen.add(k);
          return true;
        });
        if (additional.length > 0) setOccs([...quick, ...additional]);
