// v1.5.2 - 2026-03-18 - Chat 6 - pre-wrap prompt rendering, paragraph breaks in prompt text
...
async function claudeCall(prompt, maxTokens, attempt = 1, systemPrompt = null, model = "claude-haiku-4-5-20251001") {
  try {
    ...
    // Per-call fetch timeout: longer for Sonnet (large output), shorter for Haiku
    const isSonnet = model.includes("sonnet");
    const fetchTimeout = isSonnet ? 150000 : 55000;
...
CROSS-INDUSTRY ROLE RULE: The following role types exist in EVERY industry and must NEVER be assigned a single sector. This is an absolute rule with no exceptions.
Cross-industry terms: Apprentice, Trainee, Intern, Internship, Graduate, Student, Volunteer, Junior [any], Assistant [any], Trainer, Coach, Facilitator, Consultant, Analyst, Coordinator, Officer, Specialist.
MANDATORY behaviour for these terms: return multiple results each with a DIFFERENT real employing sector. Every result must have a distinct industry value. If all results share the same industry, that is always wrong.
- "Intern" must NOT all be filed under Government - return Intern across Healthcare, Finance, Technology, Marketing, Engineering, Legal etc.
- "Trainer" must NOT be filed under Education - return Corporate Trainer (Learning and Development), Safety Trainer (Manufacturing), Fitness Trainer (Sport and Wellness), Culinary Trainer (Hospitality), IT Trainer (Technology)
- "Trainee" must NOT all share one sector - return Trainee Accountant (Finance), Trainee Engineer (Engineering), Trainee Nurse (Healthcare), Trainee Chef (Hospitality)
- "Apprentice" must NOT be filed under Education - return Apprentice Electrician (Construction), Apprentice Chef (Hospitality), Apprentice Mechanic (Automotive)
...
Bad example: all "Intern" results with industry "Government and Public Administration" - interns work everywhere
Bad example: "Trainer" with industry "Education" - trainers work in every sector
Bad example: multiple results all sharing the same industry for any cross-industry term
Good example: for "Food Technologist" return Food Technologist (ISCO 3133), Food Science Technician, Quality Control Technician - all science/technical roles
Good example: for "Admin" return healthcare admin, school admin, government admin, legal admin, construction admin - spread across sectors
Good example: for "Intern" return Marketing Intern (Marketing), Finance Intern (Finance), Engineering Intern (Engineering), Healthcare Intern (Healthcare), Legal Intern (Legal) - every result a different sector
Good example: for "Trainer" return Corporate Trainer (Learning and Development), Safety Trainer (Manufacturing and Safety), Fitness Trainer (Sport and Wellness), IT Trainer (Technology), Culinary Trainer (Hospitality)`;
...
- If the search term is a cross-industry role (Apprentice, Trainee, Intern, Student, Graduate, Volunteer, Trainer, Coach, Coordinator), every result MUST have a different industry value - filing all results under one sector like Government, Education, or General is always wrong for these terms
- No invented occupations - only real ESCO v1.2 titles`, 2000, 1, SYSTEM_SEARCH);
...
`Occupation: ${title}
ISCO group: ${group}
Return exactly ${skillTarget} essential ESCO v1.2 skills for this role. Cover both technical and soft-skill types. Ensure the list reflects what a practitioner in Singapore or ASEAN actually does in this role.`, 1200, 1, SYSTEM_SKILLS);
...
Skills to rate: ${skillList}`, 4500, 1, SYSTEM_RATE);
...
PARAGRAPH RULE: The p field must use \n\n to separate each structural section of the prompt. Every prompt must have at least 3 paragraphs. Sections to separate: (1) Persona and context, (2) Technique instruction and task, (3) Output contract and constraints. HIGH prompts may have 4-5 paragraphs. Never write the prompt as one continuous block of text.
...
For l=HIGH: 1-2 sentences describing what fully automated execution of this skill looks like - written for someone who has never used AI automation before. Example: "Once you are comfortable with this prompt, you can schedule it to run automatically every Monday morning so the report lands in your inbox before your team arrives." Start with "Next phase:"
For l=MEDIUM: 1-2 sentences describing the next step toward more automation on this skill. No jargon. Start with "Next phase:"
For l=LOW: 1 sentence describing the simplest next step. No jargon. Start with "Next phase:"`;
...
Return pt exactly as assigned above. Do not substitute a different technique.`;
    ...
    try {
      const raw = await claudeCall(batchMsg, 3100, 1, SYSTEM_PROMPTS, "claude-sonnet-4-6");
...
OFFICE SUITE RULE: Skills named "Microsoft Office", "Office Suite", "Spreadsheets", "Excel", "Word", "PowerPoint" or similar general productivity suite skills must be rated MEDIUM at most - never HIGH.`;
  ...
  const raw = await claudeCall(
`Occupation: ${title}
Skills to rate: ${skillList}`, 2800, 2, SYSTEM_COMPACT);
...
Return exactly 6 realistic career progression paths for this role in Singapore and ASEAN context. Cover a genuine mix of promotion, lateral move, and specialist directions.`, 800, 1, SYSTEM_PROG);
...
Return exactly 5 crossover roles in different sectors where these skills transfer directly. Apply Singapore and ASEAN context - use job titles and sectors that are active in this market.`, 600, 1, SYSTEM_CROSS);
...
Find 5 occupations where "${skillName}" is a primary defining capability, not just incidental. Apply Singapore and ASEAN context.`, 400, 1, SYSTEM_EXPERTS);
...
Write a reflection on what stands out across these roles based only on the data provided. Do not invent information not in the data.`, 400, 1, SYSTEM_COMP);
...
Identify 5 to 6 sectors where this role is commonly found in Singapore and ASEAN. For each sector, map which skills from the list are most relevant to how this role operates there.`, 800, 1, SYSTEM_CTX);
...
function Spinner({ label, step, total }) {
  return (
    <div style={{ textAlign:"center", padding:"48px 0" }}>
      ...
      <style>{`@keyframes sp{to{transform:rotate(360deg)}} @keyframes slideUp{from{opacity:0;transform:translateX(-50%) translateY(16px)}to{opacity:1;transform:translateX(-50%) translateY(0)}} @keyframes fadeInUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <p style={{ color:C.text, fontSize:13, margin:"0 0 4px", fontWeight:600 }}>{label}</p>
      <p style={{ color:C.muted, fontSize:11, margin:"0 0 8px", lineHeight:1.5 }}>Your full skills breakdown and career paths are on the way.</p>
      {step && total && (
...
function OccupationPicker({ occs, grouped, singleSector, query, persona, pickerFullLoading, noExactMatch, onSelect, onSearchAgain }) {
  ...
  return (
    <div style={{ paddingBottom: showNudge ? 80 : 0 }}>
      ...
      <div style={{ background:C.surface, border:`2px solid ${C.accent}`, borderRadius:10, padding:"12px 16px", marginBottom:12 }}>
        <p style={{ margin:0, fontSize:16, fontWeight:800, color:C.text, lineHeight:1.3 }}>
          {pickerFullLoading
            ? <>First 8 roles for <span style={{ color:C.accent }}>"{query}"</span> <span style={{ fontSize:12, fontWeight:500, color:C.muted }}>- loading more...</span></>
            : <>{occs.length} role{occs.length!==1?"s":""} found for <span style={{ color:C.accent }}>"{query}"</span></>
          }
        </p>
        {noExactMatch && (
          <div style={{ margin:"8px 0 0", padding:"8px 10px", background:"#fffbeb", border:"1px solid #fcd9a0", borderRadius:6 }}>
            <p style={{ margin:0, fontSize:12, fontWeight:600, color:"#92400e", lineHeight:1.5 }}>
              We could not find an exact match for "{noExactMatch}" in our database.
            </p>
...
function ExposureBar({ skills }) {
  ...
  return (
    <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:"14px 18px", marginBottom:16 }}>
      <p style={{ margin:"0 0 4px", fontSize:10, color:C.muted, textTransform:"uppercase", letterSpacing:"0.07em" }}>AI Exposure Overview</p>
...
function SkillSegments({ skills, hasNoHuman }) {
  const groups = { HIGH:[], MEDIUM:[], LOW:[], HUMAN:[] };
  skills.forEach(s => { if (groups[s.level]) groups[s.level].push(s); });
  return (
    <div style={{ marginBottom:20 }}>
      <p style={{ margin:"0 0 10px", fontSize:11, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.07em" }}>Skills by Automation Segment</p>
              ...
              {items.length === 0
                ...
                : items.map((s,i) => (
                  <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:6, marginBottom:4 }}>
                    ...
                    <span style={{ fontSize:11, color:C.textSub, lineHeight:1.5 }}>{s.skill}</span>
                  </div>
                ))
              }
            </div>
...
function PromptBlock({ text, onSearch, prep, twoStep, readiness, promptTech, nextPhase, automationLevel }) {
  ...
  const NX_STYLE = {
    HIGH:   { bg:"#fdf4ff", border:"#e879f9", color:"#86198f", icon:"⚡" },
    ...
    <div style={{ marginTop:8, background:"#f0f9ff", border:"1px solid #bae6fd", borderRadius:7, padding:"10px 12px" }}>
      {prep && (
        <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:8, padding:"6px 10px", background:"#fffbeb", border:"1px solid #fcd9a0", borderRadius:5 }}>
          ...
          <p style={{ margin:0, fontSize:10, color:"#92400e", lineHeight:1.5, fontStyle:"italic" }}>{prep}</p>
        </div>
      )}
      {twoStep && (
      ...
      <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap", marginBottom:6 }}>
          <p style={{ margin:0, fontSize:10, fontWeight:700, color:"#0369a1", textTransform:"uppercase", letterSpacing:"0.06em" }}>Prompt</p>
      ...
      {nextPhase && (() => {
        const nxText = nextPhase.replace(/^Next phase:\s*/i, "");
        const nxDisplay = nxText.charAt(0).toUpperCase() + nxText.slice(1);
        return (
          <div style={{ marginTop:10, padding:"11px 14px", background:nxStyle.bg, border:`1px solid ${nxStyle.border}`, borderRadius:7 }}>
            <p style={{ margin:"0 0 5px", fontSize:13, fontWeight:700, color:nxStyle.color }}>What to do next</p>
            <p style={{ margin:0, fontSize:13, color:nxStyle.color, lineHeight:1.7 }}>{nxDisplay}</p>
          </div>
        );
      })()}
...
function SkillGroupedView({ grouped, result, onSearch, skillInputResult, skillInputQuery, onSkillSearch, onSkillQueryChange, firstAnalysis, onQueue, queueCount, currentRole }) {
    ...
    <div>
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:"10px 14px", marginBottom:12 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
          <p style={{ margin:0, fontSize:12, fontWeight:700, color:C.muted }}>Skill Analysis ({result.skills?.length||0} skills)</p>
        ...
        <p style={{ margin:0, fontSize:12, color:C.textSub, lineHeight:1.6 }}>Sorted by your strengths first. Tap any group header to expand or collapse.</p>
        <p style={{ margin:"5px 0 0", fontSize:12, color:C.muted, fontStyle:"italic" }}>Ratings reflect general role exposure, not seniority level.</p>
      </div>
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:"10px 14px", marginBottom:14 }}>
        <p style={{ margin:"0 0 4px", fontSize:12, fontWeight:700, color:C.text }}>Where does your skill fit in this role?</p>
        <p style={{ margin:"0 0 8px", fontSize:12, color:C.muted, lineHeight:1.5 }}>Type a skill you have and we will show you how it maps here. In English for best results.</p>
        <div style={{ display:"flex", gap:8 }}>
          <input type="text" value={skillInputQuery} onChange={e => onSkillQueryChange(e.target.value)} onKeyDown={e => e.key === "Enter" && onSkillSearch(skillInputQuery)} placeholder="e.g. facilitation, Excel, managing conflict..."
          ...
          <button onClick={() => onSkillSearch(skillInputQuery)} disabled={!skillInputQuery.trim()}
            ...
            <div style={{ border:`1px solid ${g.border}`, borderTop:"none", borderRadius:"0 0 7px 7px", padding:"8px 8px 4px", background:C.bg }}>
              ...
              {g.skills.map((s, i) => {
                ...
                <SkillRow key={i} item={s} idx={i}
                  ...
                  onSearch={onSearch}
                  autoOpen={firstAnalysis && gIdx === firstAiGroupIdx && i === 0}
                  onQueue={onQueue}
...
// Skill detail row
function SkillRow({ item, idx, onSearch, highlight, autoOpen, matchRef, onQueue, queueCount, currentRole }) {
  ...
  return (
    <>
      ...
      <div ref={matchRef || null} onClick={() => setOpen(o => !o)} style={{ border:`2px solid ${highlight ? c.border : open ? c.border : C.border}`, borderRadius:7, marginBottom:5, background: highlight ? c.bg : open ? c.bg : C.surface, cursor:"pointer", transition:"all 0.15s", boxShadow: highlight ? `0 0 0 3px ${c.bg}` : "none" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 13px" }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <p className="t-body" style={{ margin:0, fontSize:13, color:C.text, fontWeight:500 }}>{item.skill}</p>
            <p style={{ margin:"1px 0 0", fontSize:12, color:C.muted }}>{item.type} - {item.skillType === "soft-skill" ? "Soft Skill" : "Technical Skill"}</p>
          </div>
        ...
        {open && (
          <div style={{ padding:"2px 13px 11px 41px", borderTop:`1px solid ${c.border}` }}>
            <p style={{ margin:"8px 0 7px", fontSize:12, color:c.color, fontWeight:600 }}>
              {c.icon} {item.tool === "NA" ? "Note:" : "Kickstart:"} {item.kickstart || AI_USAGE[item.tool] || ""}
            </p>
            ...
            {item.level !== "HUMAN" && (
              item.promptLoading
                ? <div style={{ marginTop:8, padding:"10px 12px", background:"#f0f9ff", border:"1px solid #bae6fd", borderRadius:7, display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <p style={{ margin:0, fontSize:11, color:"#0369a1", fontStyle:"italic" }}>Generating sophisticated prompt...</p>
                  </div>
                : item.promptFailed
                  ...
                  : item.prompt
                    ? <PromptBlock text={item.prompt} onSearch={onSearch} prep={item.prep||""} twoStep={item.twoStep||false} readiness={item.readiness||"ready"} promptTech={item.promptTech||""} nextPhase={item.nextPhase||""} automationLevel={item.level} />
                    : null
            )}
            {/* Who uses this skill */}
            <button
            ...
            {isOpen && (
              <div style={{ padding:"4px 16px 12px 60px", borderTop:"1px solid #a5f3fc" }}>
                ...
                {sectorSkills.length > 0
                  ...
                  : <p style={{ margin:0, fontSize:12, color:C.muted, fontStyle:"italic" }}>See Skill Detail tab for the full breakdown.</p>
                }
              </div>
            )}
          </div>
...
function CategoryPanel({ skills }) {
  const lvlOrd = { HIGH:0, MEDIUM:1, LOW:2, HUMAN:3 };
  const soft = [...skills.filter(s => s.skillType === "soft-skill")].sort((a,b) => (lvlOrd[a.level]??2)-(lvlOrd[b.level]??2));
  const tech = [...skills.filter(s => s.skillType === "technical")].sort((a,b) => (lvlOrd[a.level]??2)-(lvlOrd[b.level]??2));
  return (
    <div>
      {/* Header */}
      <div style={{ background:C.tealBg, border:`1px solid ${C.tealBdr}`, borderRadius:8, padding:"10px 14px", marginBottom:14 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3 }}>
          <p style={{ margin:0, fontSize:12, fontWeight:700, color:C.teal }}>Skill Categories</p>
          <div style={{ display:"flex", gap:12 }}>
            <span style={{ fontSize:12, color:C.accent, fontWeight:600 }}>{tech.length} Technical</span>
            <span style={{ fontSize:12, color:C.purple, fontWeight:600 }}>{soft.length} Soft</span>
          </div>
        </div>
        <p style={{ margin:"3px 0 0", fontSize:12, color:C.textSub, lineHeight:1.6 }}>
          Skills grouped by their primary nature. Most roles blend both - technical skills tend to be more exposed to AI automation, while soft skills are generally more resilient.
        </p>
        <p style={{ margin:"6px 0 0", fontSize:12, color:C.muted, fontStyle:"italic" }}>
          Some skills have characteristics of both categories - the grouping here reflects the dominant nature of each skill.
        </p>
      </div>
      {/* Two column skill list */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(min(260px, 100%), 1fr))", gap:14 }}>
        <div>
          <p style={{ margin:"0 0 8px", fontSize:11, fontWeight:700, color:C.accent, textTransform:"uppercase", letterSpacing:"0.06em" }}>
            Technical Skills ({tech.length})
          </p>
          {tech.length === 0
            ? <p style={{ fontSize:12, color:C.mutedLight, fontStyle:"italic" }}>None identified</p>
            : tech.map((s,i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:5, padding:"6px 10px", background:C.surface, border:`1px solid ${C.border}`, borderRadius:6, minWidth:0 }}>
                <div style={{ width:112, flexShrink:0 }}><Tag level={s.level} small /></div>
                <span style={{ fontSize:12, color:C.textSub, minWidth:0, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{s.skill}</span>
              </div>
            ))
          }
        </div>
        <div>
          <p style={{ margin:"0 0 8px", fontSize:11, fontWeight:700, color:C.purple, textTransform:"uppercase", letterSpacing:"0.06em" }}>
    ...
    <div style={{ marginTop:14, paddingTop:12, borderTop:`1px solid ${C.border}` }}>
      ...
      {open === "method" && (
        <div style={{ marginTop:12, background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:"16px 20px" }}>
          ...
          <p style={{ margin:"0 0 10px", fontSize:12, color:C.textSub, lineHeight:1.7 }}>
            <strong style={{ color:C.text }}>Data source</strong> - Occupation and skills taxonomy drawn from ESCO v1.2 (aligned to v1.2.1), the European Classification of Skills, Competences, Qualifications and Occupations. Published by the European Commission DG Employment, Social Affairs and Inclusion. Licensed CC BY 4.0.
          </p>
          <p style={{ margin:"0 0 10px", fontSize:12, color:C.textSub, lineHeight:1.7 }}>
      ...
      {open && (
        <div style={{ padding:"4px 16px 12px 62px", borderTop:`1px solid ${d.border}` }}>
          ...
          {relevantSkills.length > 0
            ...
            : <p style={{ margin:0, fontSize:12, color:C.muted, fontStyle:"italic" }}>See Skill Detail tab for the full skills breakdown.</p>
          }
          <p style={{ margin:"8px 0 0", fontSize:12, color:C.muted, fontStyle:"italic" }}>
            Based on your current role's highest-automation skills. See Skill Detail tab for the full breakdown.
          </p>
          ...
          <div style={{ display:"flex", gap:8, marginTop:10, flexWrap:"wrap" }}>
            <button
              ...
              style={{ padding:"5px 14px", fontSize:12, fontWeight:700, color:"#fff", background:d.color, border:"none", borderRadius:6, cursor:"pointer", display:"inline-flex", alignItems:"center", gap:5 }}>
              Analyse this role &#8594;
            </button>
    ...
    <div onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <div style={{ padding:"4px 16px 12px 62px", borderTop:`1px solid ${C.greenBdr}` }}>
          ...
          <p style={{ margin:"8px 0 0", fontSize:12, color:C.muted, fontStyle:"italic" }}>
            Human-led and AI-assisted skills tend to transfer best across sectors. See Skill Detail tab for the full breakdown.
          </p>
          ...
          <div style={{ display:"flex", gap:8, marginTop:10, flexWrap:"wrap" }}>
            <button
              onClick={e => { e.stopPropagation(); onAnalyse(item.role); }}
              style={{ padding:"5px 14px", fontSize:12, fontWeight:700, color:"#fff", background:C.green, border:"none", borderRadius:6, cursor:"pointer", display:"inline-flex", alignItems:"center", gap:5 }}>
              Analyse this role &#8594;
            </button>
            {onQueue && onQueueCount < 3 && (
              <button
...
export default function App() {
  ...
  const [pickerFullLoading, setPickerFullLoading] = useState(false); // v1.3.0: background full search
  ...
  const [fromPath, setFromPath] = useState(null); // "progression" | "crossover" | null
  ...
  // v6: debounced instant-search — fires 420ms after user stops typing
  // Only active on idle/error step so it doesn't fire during analysis
  // Uses quick 8-result search only - full search is doSearch's responsibility
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await searchOccupations(q, "8");
        if (!cancelled) setOccs(res);
  ...
  const reset = () => { pickerCancelRef.current = true; setNoExactMatch(null); setStep("idle"); setOccs([]); setSel(null); setResult(null); setErr(""); setQuery(""); setSub(""); setSubStep(0); setActiveTab("skills"); comparisonsRef.current = []; setComparisons([]); setCompareCue(false); setFromPath(null); };
  ...
  const softReset = (savedComparisons) => {
    ...
    comparisonsRef.current = savedComparisons; setComparisons(savedComparisons); setFromPath(null);
  };
  ...
  const doSearch = useCallback(async () => {
    ...
    // v6: if debounce already fetched results, use them immediately
    ...
    setNoExactMatch(null);
    try {
      // v1.3.0: quick search returns 8 results fast - show picker immediately
      const quick = await searchOccupations(tidyQuery, "8");
      ...
      searchOccupations(tidyQuery, "15 to 20").then(full => {
        if (thisCancel.current) { setPickerFullLoading(false); return; }
        if (full.length > quick.length) {
          const quickTitles = new Set(quick.map(o => o.title.toLowerCase()));
          const additional = full.filter(o => !quickTitles.has(o.title.toLowerCase()));
          setOccs([...quick, ...additional]);
        }
        setPickerFullLoading(false);
      }).catch(() => setPickerFullLoading(false));
    } catch(e) { setErr(e.message); setStep("error"); }
  }, [query, occs, pickerLoading]);

  const doAnalyse = useCallback(async (occ) => {
    setSel(occ); setStep("loading"); setSub(`Finding the essential skills for ${toTitleCase(occ.title)}...`); setSubStep(1); setResult(null); setErr("");
    setShowExpect(false);
    ...
    try {
      const skills = await getSkills(occ.title, occ.iscoGroup || "", occ.iscoCode || "");
      ...
      const merged = skills.map(s => {
        ...
        return { n:s.n, skill:s.skill, type:s.type, level:r.level||"HUMAN", tool:r.tool||"NA", how:r.how||"", kickstart:r.kickstart||"", prompt:"", promptTech:"", nextPhase:"", promptLoading:r.level !== "HUMAN", promptFailed:false, skillType:r.skillType||"technical", prep:r.prep||"", twoStep:r.twoStep||false, readiness:r.readiness||"ready" };
      });
      ...
      const newResult = { iscoGroup:occ.iscoGroup||"", description:occ.description||"", skills:merged, foundationData, progressionData, crossoverData, contextData };
      setResult(newResult);
      ...
      const promptTimeout = new Promise((_, reject) =>
        setTimeout(() => reject(new Error("prompt_timeout")), 60000)
      );
    ...
    } catch(e) { analysisComplete = true; clearTimeout(safetyTimer); setErr(e.message); setStep("error"); }
  }, [persona]);
  ...
  const handleAnalyseRole = useCallback(async (roleTitle, pathType) => {
    const doIt = async () => {
      ...
      setQuery(tidyRole);
      setFromPath(pathType || "progression");
      window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  useEffect(() => {
    if (step === "results" && !hasAnalysedOnce.current) {
      const id = setTimeout(() => { hasAnalysedOnce.current = true; }, 800);
      return () => clearTimeout(id);
    }
  }, [step]);

  // Back-to-top: show button after scrolling 400px
  useEffect(() => {
    const onScroll = () => setShowBackTop(window.scrollY > 400);
    window.addEventListener("scroll", onScroll, { passive: true });
    return () => window.removeEventListener("scroll", onScroll);
...
Identify if the input matches or relates to any skill in the list.
Return ONLY a JSON object. No text before or after. No markdown.
{"match":"exact skill name from list or empty string","close":"closest skill name if no exact match or empty","explanation":"one sentence about this skill in this role context","suggestion":"if input unclear or not in English - a gentle plain English clarification request","unrelated":false}
Keep all values under 30 words. No quote characters inside values.`, 280);
  ...
  const runQueuedComparisons = useCallback(async () => {
    ...
    const step = (msg) => { globalStep++; setCompareStep(globalStep); setCompareStatus(msg); };

    const analyseOne = async (c, roleIndex) => {
      try {
        const roleLabel = toTitleCase(c.title);
        step(`Finding essential skills for ${roleLabel}...`);
        ...
        occ.title = c.title;
        const skills = await getSkills(occ.title, occ.iscoGroup || "", occ.iscoCode || "");
        step(`Rating ${skills.length} skills for ${roleLabel} against AI...`);
        ...
        const merged = skills.map(s => {
          const r = ratings.find(x => x.n === s.n) || {};
          return { n:s.n, skill:s.skill, type:s.type, level:r.level||"HUMAN", tool:r.tool||"NA", how:r.how||"", kickstart:"", prompt:"", skillType:r.skillType||"technical", prep:"", twoStep:false, readiness:"ready" };
        });
        step(`Mapping career paths for ${roleLabel}...`);
  ...
  const buildTabs = (r) => {
    if (!r) return [];
    return [
      { key:"skills",      label:"📋 Skill Detail",         color:C.muted   },
  ...
  return (
    ...
    <div data-author="Adrian K. L. Ang" data-origin="takearoundabout.com" data-build="v5-2026"
      ...
      <div className="main-content">
          ...
          <>
            ...
            <div style={{ background:C.surface, border:`2px solid ${C.accent}`, borderRadius:10, padding:14, marginBottom:10 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                ...
                <button onClick={doSearch} style={{ background:C.eu, border:"none", borderRadius:7, color:"#fff", padding:"11px 22px", fontSize:13, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap" }}>
                  Search
                </button>
              </div>
        ...
        {step === "picking" && (() => {
          ...
          <OccupationPicker
            ...
            onSearchAgain={async (newQuery) => {
              ...
              setPickerFullLoading(false);
              try {
                const quick = await searchOccupations(tidy, "8");
                if (!quick.length) { setErr("no occupations found"); setStep("error"); return; }
                if (quick.length === 1) { doAnalyse(quick[0]); return; }
                setOccs(quick); setStep("picking");
                setPickerFullLoading(true);
                searchOccupations(tidy, "15 to 20").then(full => {
                  if (full.length > quick.length) {
        ...
        {step === "loading" && <Spinner label={sub || "Loading..."} step={subStep} total={persona ? 4 : 3} />}
          ...
          return (
            <div>
              <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:"15px 18px", marginBottom:16 }}>
                ...
                {result.iscoGroup && <p style={{ margin:"6px 0 0", fontSize:10, color:C.mutedLight }}>ESCO v1.2.1 · {result.iscoGroup}</p>}
              ...
              <SkillSegments skills={result.skills} hasNoHuman={result.skills.every(s => s.level !== "HUMAN")} />

              <div ref={tabBarRef} style={{ marginBottom:14 }}>
                <p style={{ margin:"0 0 8px", fontSize:11, color:C.muted }}>
                  Tap or click a tab below to explore the results:
                </p>
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                  {tabs.map(t => {
                    const readyCount = comparisons.filter(c => c.result && c.result.skills).length;
                    ...
                    return (
                    <button key={t.key}
                      onClick={() => { if (!compareDisabled) { setActiveTab(t.key); track("tab_viewed", { tab: t.key }); } }}
