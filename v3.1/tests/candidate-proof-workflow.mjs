...
//
// It does NOT re-assert the per-slice behaviours those five suites already own. Re-asserting them
// would raise this suite's count without raising coverage, which is the count-inflation the
// Blueprint Supervisor has already corrected twice on this programme.
//
...
//
// PROVENANCE NOTE. This file imports BLP028_VIEWPORTS and assertMatrixSurface from
// tests/support/blp028-matrix.mjs, which entered the repository at 61531dc (PR #508) without a
// scope ruling and which the Supervisor's #508 ruling records as material, not as provenance.
// Reuse was approved for this requirement so that a second geometry helper would not drift from
// the first. Importing it verifies NOTHING about it: every assertion below is this suite's own.
//
...
async function pressByKeyboard(locator, page, key = "Enter", label = "") {
  ...
  const focused = await page.evaluate(() => {
    const a = document.activeElement;
    return { testid: a?.dataset?.testid || null, tag: a?.tagName || null, disabled: a?.disabled === true };
  });
  ok(focused.testid !== null && !focused.disabled, `${label}: the control takes keyboard focus and is not disabled before the key is sent (focus was ${JSON.stringify(focused)}); without this the key would go to the document and the stage would pass without operating the control`);
...
const PAIRS = [
  { a: '[data-testid="cpl-declare-demonstrated"]', b: '[data-testid="cpl-declare-certified"]', states: ["CLAIMED_ONLY"], why: "the two state declarations sit side by side at phone width and must both be readable and pressable" },
  { a: '[data-testid="cpl-claim-state"]', b: '[data-testid="cpl-claim-save"]', states: null, why: "the announced claim outcome and the control that produces it must be readable together, or a keyboard user hears an outcome they cannot see" },
  { a: '[data-testid="cpl-dest"][data-destination="resume"] [data-testid="cpl-dest-approve"]', b: '[data-testid="cpl-dest"][data-destination="resume"] [data-testid="cpl-dest-revoke"]', states: null, why: "approve and revoke are adjacent on one destination row and pressing the wrong one is not recoverable by undo" },
  { a: '[data-testid="cpl-record-state"]', b: '[data-testid="cpl-excerpt"]', states: null, why: "the state of a record and the text it was cut from must be read together, or the state is read against the wrong excerpt" },
];

async function assertPairsReadable(page, scope, tag, expectedState) {
  ...
  const applicable = PAIRS.filter((p) => p.states === null || p.states.includes(expectedState));
...
const LEDGER_CONTROLS = ["cpl-declare-demonstrated", "cpl-declare-certified", "cpl-claim-input", "cpl-claim-save", "cpl-type-select", "cpl-link-select", "cpl-link-button", "cpl-dest-approve", "cpl-dest-revoke", "cpl-link-open", "cpl-unlink"];

async function assertControlsReachable(page, tag, stage, anchorTestId = "cpl-record") {
  // The ledger is taller than any viewport, so the controls must be brought to where a human would
  // be looking before "is anything covering this?" is a meaningful question. Without this the check
  // finds nothing in view and its own guard below fires, which is how the first run of this suite
  // caught itself being vacuous.
  await page.evaluate((id) => { document.querySelector(`[data-testid="${id}"]`)?.scrollIntoView({ block: "center" }); }, anchorTestId);
  ...
  const covered = await page.evaluate((ids) => {
    ...
    for (const id of ids) {
      for (const el of document.querySelectorAll(`[data-testid="${id}"]`)) {
        ...
        // Sample the VISIBLE box, not the raw box. A control straddling the edge of a scrolling
        // panel has a centre that is not painted anywhere, and elementFromPoint there returns an
        // ANCESTOR - which would read as "covered" while nothing is on top of it. The first run of
        // this suite reported exactly that, and it was the instrument, not the layout.
        ...
        // alone was not enough and the second run still reported an ancestor at the sample point.
        ...
        if (visible < r.width * r.height * 0.5 || right - left < 8 || bottom - top < 8) { clipped += 1; continue; }
        seen += 1;
        ...
        bad.push({ control: id, coveredBy: hit ? (hit.dataset?.testid || `${hit.tagName}.${String(hit.className).slice(0, 40)}`) : "nothing", kind });
      }
    }
    return { seen, clipped, bad };
  }, LEDGER_CONTROLS);
  ok(covered.seen > 0, `${tag} ${stage}: at least one ledger control is in view for the obscuring check (a zero here would make the next assertion vacuous, which is the shape this programme keeps correcting)`);
  ok(covered.bad.length === 0, `${tag} ${stage}: no ledger control in view is covered by another element; ${covered.seen} judged, ${covered.clipped} skipped as less than half visible, covered: ${JSON.stringify(covered.bad)}`);
}
...
const CARRIED = ["proofId", "sourceId", "spanId", "excerpt", "recordCount"];
...
async function runViewport({ name, width, height, phone }) {
  ...
  await assertControlsReachable(page, tag, "stage 1 (records minted)");
  await assertPairsReadable(page, rec0, `${tag} stage 1`, "CLAIMED_ONLY");
  ...
  ok(/Claim recorded/.test(await rec0.getByTestId("cpl-claim-state").innerText()), `${tag} stage 2: the element that took focus announces the outcome (fails if focus lands somewhere that says nothing)`);
  now = await readAnchors(page);
  ...
  now = await readAnchors(page);
  assertAnchors(claimed, now, [...CARRIED, "claim", "linkId", "linkTargetId"], tag, "stage 2 after choosing the proof type by keyboard");
  ...
  await assertControlsReachable(page, tag, "stage 2 (record fully populated)");
  await assertPairsReadable(page, rec0, `${tag} stage 2`, "DEMONSTRATED");
  ...
  eq(originLinkId, beforeRoundTrip.linkId, `${tag} stage 3: the control about to be pressed belongs to the link the anchors describe`);
  ...
  await assertControlsReachable(page, tag, "stage 3 (after the evidence round trip)");
  await assertPairsReadable(page, rec0, `${tag} stage 3`, "DEMONSTRATED");
  ...
  await assertControlsReachable(page, tag, "stage 4 (after the print round trip)");

  // -------------------------------------------------------------------------------------------
  // Stage 5. The whole session read once more, so the end is compared with the beginning rather
  // than only with the stage before it. A drift introduced and reverted stage by stage would pass
  // every step above and fail here.
  // -------------------------------------------------------------------------------------------
  const ended = await readAnchors(page);
  assertAnchors(minted, ended, CARRIED, tag, "stage 5 end of session against the moment the records were minted");
...
console.log([
  ...
  "  - the per-slice behaviours of BLP-007 to BLP-011; each is owned and asserted by its own suite,",
  "    and re-asserting them here would raise the count without raising coverage.",
  "  - a second opening of the print package in one session. The legacy studio applies each intent",
  "    by its JSON key, so a second { kind: \"print\" } is the same key and lands on the workspace",
  "    with no overlay. Pre-existing, not introduced here, and raised to the Blueprint Supervisor",
  "    rather than fixed inside a verification requirement.",
  "  - physical-device runtime. completionPolicy.requiresPhysicalRuntime is false for BLP-012, so",
  "    four emulated widths are the evidence and no claim is made about real hardware.",
  "  - the posting side of the round trip. This suite returns from the evidence workspace to the",
