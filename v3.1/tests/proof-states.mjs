...
ok(missingEvidenceOf(rec(res.ledger, B), { bundle }).some((m) => /no link stands while this record is WITHHELD: 1 link waits on the record, not on the posting/.test(m)) && !missingEvidenceOf(rec(res.ledger, B), { bundle }).some((m) => /invalid against the current posting evidence/.test(m)), "the withheld side's missing evidence says its link waits on the record, never that it is invalid against posting evidence that did not move (conformance-auditor, third pass: fails if the fallback narrates a candidate-side reason as a target-side one)");
ok(missingEvidenceOf(rec(fullLoss, A), { bundle: bundle2 }).some((m) => /no valid link to a target: 2 links are invalid against the current posting evidence/.test(m)), "and a record whose links were lost on the target side still says so, from the links' own reasons");
