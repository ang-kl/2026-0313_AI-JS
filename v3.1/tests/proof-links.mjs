...
const notRejudged = guardLedgerStep(faultedTwice, (l) => l, t(7));
ok(notRejudged.faults.length === 2 && notRejudged.faults.every((f) => f.resolvedAt === null) && ledgerFaultText(notRejudged) !== null, "a step that succeeds WITHOUT re-judging the links (a claim edit, a proof-type choice) leaves the open faults open: the sentence says nothing is current until a fold or reconcile validates, and only those may lift it (conformance-auditor W-NEW-1 on BLP-010)");
const recovered = guardLedgerStep(faultedTwice, (l) => reconcileLinks(l, bundle2, t(7)), t(7), { rejudged: true });
