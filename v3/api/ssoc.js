...
export default async function handler(req, res) {
  ...
  if (action === 'seed') {
    try {
      return res.status(200).json(await seedDatabase());
    } catch (err) {
      console.error('[ssoc] seed:', err && err.message);
      return res.status(200).json({ ok: false, db: false, error: err.message, counts: loadFlat().counts });
    }
  }
