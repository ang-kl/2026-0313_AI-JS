...
|---|---|---|---|---|---|
| **FR1** | goal 7 Forensic Reversal | ADDITIVE | `App.jsx` | goal 7; paper SS1, SS7 | SHIPPED v3.1.4: verb-mandate + crux anomaly render on Metta/NHG/PSD with honest chips; LLM authors no number that reaches the page |
| **BF2** | goal 1 Ontological Parsing | ADDITIVE | `LeapView.jsx` | goal 1; paper SS1; Rumelt kernel | SHIPPED v3.1.5: bridge/firewall verdict, `~ AI estimate`, never presented as computed (word-balance per the AU-7 below) |
| **RK1** | goal 1 Ontological Parsing (Rumelt kernel - the missing half) | NEW panel | `App.jsx` | goal 1; Rumelt, *Good Strategy / Bad Strategy* (2011) | SHIPPED v3.2.1: "Strategy read" - diagnosis (the obstacle the vacancy signals) / guiding policy / coherent action, grounded ONLY in the role's duties; fully `~ AI estimate`, authors NO number (friction cost NOT built - no data); coherent actions must cite a real duty or are dropped; withheld under 3 duties / on empty diagnosis. Mirrors the governed FR1 pattern; D1-D8 8/8 PASS |
| **BDF3** | goal 5 BDF | NEW panel | `App.jsx` | goal 5; paper SS4 | SHIPPED v3.1.6: boundary (not-to-do) / N-1 -> N+1 map / feedback risks, all `◐ derived` (as built: `~ AI estimate` per the AU-7 below) |
| **PW4** | w34854 | REWIRE-lite | `App.jsx` help copy | w34854 task categories | SHIPPED v3.1.7: each automation level carries its PWAI citation; zero score change (A8 idiom) |
| **ST3** | goal 3 Stewardship shift | NEW panel | `App.jsx` | goal 3; paper SS3; Taleb antifragility | SHIPPED v3.1.8: "Where you sit" - human-keeps vs AI-takes, deterministic composition of skill levels + anatomy layers, ~ AI estimate, no number |
| **RB1** | goal 3 Stewardship shift (foundation layer) | REWIRE (rubric only) | `App.jsx` | goal 3 ("procedural execution commoditised by autonomous systems; humans = legal personhood, moral liability, intuition"); w34854 crosswalk unchanged | SHIPPED v3.3.0: the 4-level skill-analysis rubric re-baselined to the agentic frontier in all THREE raters (SYSTEM_RATE, SYSTEM_COMPACT, SYSTEM_RR) - HIGH = AI/agent completes end-to-end, human reviews the OUTCOME; HUMAN = the governance node (protocol-3 wording, same family ST3 uses). AGENT tool code added; Step-3 "Automate it" = the agent handover (brief, checkpoint, escalation) matching ST3's "Hand to AI agents" pole; help copy corrected to the honest FOUR levels. NO 5th level, NO label/colour change; advisory calls on Fable 5 (v3.2.10). The foundation layer now agrees with the panels built on it |
