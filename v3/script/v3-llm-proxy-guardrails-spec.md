...
1. **Anthropic Messages API** — `POST https://api.anthropic.com/v1/messages`. Selected when `CLAUDE_API_KEY` is set. Model resolved by `anthropicModelFor(model)` — always resolves to `claude-sonnet-5` regardless of the caller's requested tier, unless `ANTHROPIC_MODEL` is explicitly set (operator override). Auth via `x-api-key` + `anthropic-version: 2023-06-01` headers.
2. **OpenAI Responses API** — `POST https://api.openai.com/v1/responses`. Not in the built-in default chain (Anthropic-only, see below) — only tried if an admin KV override re-adds it, AND `OPENAI_API_KEY` is set. Model resolved by `openAiModelFor(model)`.
3. **Google Gemini** — `POST https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent`. Not in the built-in default chain either — only tried if an admin KV override re-adds it, AND `GEMINI_API_KEY` + `GEMINI_MODEL` are set. Model comes from `GEMINI_MODEL` env; `models/` prefix stripped.
4. **No further fallback.** If every configured/enabled path fails, the client sees a `503` with a warm error message (`WARM_ERRORS.server` or `WARM_ERRORS.timeout` / `overload` as appropriate).

**Built-in default chain (v3.0.269+): Anthropic only.** Human Lead directive 09-07 '26 — Gemini and OpenAI free-tier quotas proved too unreliable for this app's per-analysis call volume (confirmed live: Gemini genuinely quota-limited per its own error, OpenAI `insufficient_quota`). `VALID_PROVIDERS` still recognizes all three, so the Admin Panel can reorder/re-add Gemini or OpenAI as fallbacks without a code change.
...
**Cold-start correctness:** if `CLAUDE_API_KEY`, `OPENAI_API_KEY`, and (`GEMINI_API_KEY` + `GEMINI_MODEL`) are all missing, the endpoint returns `500` immediately with the `No LLM provider configured` debug string. It does not silently accept requests it cannot serve.
...
| `[proxy] no LLM provider configured: CLAUDE_API_KEY / OPENAI_API_KEY / (GEMINI_API_KEY + GEMINI_MODEL) all missing` | none — env state |
...
**Auth failures (`401`/`403` from upstream)** are converted to `503 AUTH` with a message that names the exact env var to check (`CLAUDE_API_KEY`, `OPENAI_API_KEY`, or `GEMINI_API_KEY` depending on which provider failed). This is user-facing UX; it is safe because the message names the *variable*, not any *value*.
...
- **2026-07-06 — Anthropic-first.** `callAnthropic()` added and wired as primary in the fallback chain. New order: **ANTHROPIC → OPENAI → GEMINI.** Env vars added: `CLAUDE_API_KEY` (named `ANTHROPIC_API_KEY` at the time), optional `ANTHROPIC_MODEL`, `ANTHROPIC_MODEL_STRONG` (default `claude-opus-4-8`), `ANTHROPIC_MODEL_FAST` (default `claude-haiku-4-5-20251001`). Response envelope shape and cap set unchanged.
- **2026-07-09 — Anthropic-only, Sonnet 5 only, `CLAUDE_API_KEY` rename.** Human Lead directive: Gemini/OpenAI free-tier quotas proved too unreliable for this app's per-analysis call volume (confirmed live via server logs). `DEFAULT_CHAIN` → `["anthropic"]` only; Gemini/OpenAI stay configured but excluded unless an admin KV override re-adds them. `anthropicModelFor()` now always resolves to `claude-sonnet-5` regardless of the caller's requested tier (haiku/opus/fable), unless `ANTHROPIC_MODEL` is explicitly set. The Anthropic key env var was renamed in Vercel from `ANTHROPIC_API_KEY` to `CLAUDE_API_KEY`; the proxy and this spec were updated to match — `ANTHROPIC_API_KEY` is no longer read anywhere.
