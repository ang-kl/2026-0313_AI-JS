...
| CJ2 | Task Prep ("Arm") | ADDITIVE | `App.jsx` | the extracted duties (responsibilitiesData + jobAnatomy) | deterministic assembler: per task = what you'd do (● MCF) / why (layer+exposure) / how to prepare (per-skill how/kickstart via `sk` refs); no invented task; new Task Prep tab; withhold when no duties |
| CJ3 | Interview Rehearsal | ADDITIVE | `App.jsx` | STARs; Schmidt-Hunter 1998 | SYSTEM_REHEARSE: real-duty interview question + EMPTY STAR scaffold (model never writes the candidate's answer); `~ AI estimate`; new Interview Prep tab; withheld under thin duties; stands alone (no Resume dependency) |
| CJ4 | Journey storyboard spine | ADDITIVE | `App.jsx` | - | pure-UI strip in the Navigation box: 5 stations, deterministic readiness (ready/locked/done by shape+label+number, not colour), tap -> setActiveTab; "locked" until its data exists; Resume not a station |
